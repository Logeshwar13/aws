...
class _AppBarWidgetState extends State<AppBarWidget>
  ...
  void didUpdateWidget(AppBarWidget oldWidget) {
    ...
    if (widget.showAdmin != oldWidget.showAdmin) {
      if (widget.showAdmin) {
        _routes.add('/admin/login');
      } else {
        _routes.remove('/admin/login');
      }
  ...
  Widget _desktopTabBar(BuildContext context, ThemeData theme, bool isDark) {
    final tabs = [
      ...
      if (widget.showAdmin) const Tab(text: 'Admin'),
    ];

    return SizedBox(
      width: widget.showAdmin ? 850 : 750,
      child: Theme(
        data: theme.copyWith(
